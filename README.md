...
요구사항 외에, 사용성을 높이기 위해 추가로 구현한 기능입니다.

### 📌 항목 상단 고정 (Pin)

- 각 항목의 **핀 버튼**으로 중요한 할 일을 목록 **최상단에 고정**할 수 있습니다.
- 고정 목록은 `localStorage`에 저장되어 **새로고침/재방문 후에도 유지**됩니다.
- 정렬은 **고정 항목이 항상 먼저** 노출되며, 고정 그룹의 안과 밖 모두 기본 정렬
  (미완료 우선 → 기한 임박순)을 그대로 따릅니다.
- 언제든 고정 해제할 수 있고, 고정 상태는 **채워진 핀 아이콘(강조색)** 과 외곽선
  아이콘으로 구분합니다.
- 구현: store의 `pinnedIds`(영속) + `useTodoListView`의 정렬 `select`에서 고정 우선
  비교. 항목은 자신의 고정 여부만 구독해 다른 항목을 리렌더하지 않습니다.

> 이 밖에 **다크 모드 · 반응형 카드 레이아웃 · 검색 디바운스/즉시 해제** 등도 요구사항
> 외 개선으로 포함되어 있습니다(위 *비기능 요구사항* 및 *아키텍처* 참고).
